...
- **TTL de reserva:** cada reserva guarda un campo `expira_en`. Un proceso libera automáticamente las reservas vencidas (`RESERVADA → DISPONIBLE`), para que un checkout abandonado no congele el stock. El TTL es **configurable** (vive en Settings, ver §5.3). El *mecanismo* se construye en este bloque; el *valor* fino se ajusta en el bloque 6, cuando exista el pago real con su ventana (Webpay / Mercado Pago).
...
### 5.3 Reglas
- Los multiplicadores (venta 1.000, compra 400), el tipo de cambio USD→CLP, la regla de redondeo y el TTL de reserva son **parámetros de negocio editables en runtime**. Viven en una **tabla `Settings` en la BD**, editable desde la interfaz por el administrador de tienda — **no** en variables de entorno (esas exigen acceso al servidor y reinicio, lo que sí es "tocar el sistema"). Regla general: **secretos → variables de entorno** (`DATABASE_URL`, `JWT_SECRET`, credenciales); **parámetros de negocio → tabla Settings**.
...
- [ ] Una reserva con `expira_en` vencido es **liberada automáticamente** (vuelve a `DISPONIBLE`) por el proceso de expiración.
...
- [ ] **Cargar un bulk:** se ingresa el costo total y el sistema distribuye el costo por carta ponderado por su **precio de venta**, y muestra el margen por carta.
- [ ] Todo movimiento de stock queda **registrado** (tipo, usuario, fecha, cantidad).
- [ ] Los parámetros de negocio (multiplicadores, USD→CLP, redondeo, TTL) se editan en **runtime desde la tabla Settings**, sin tocar el servidor.
...
- **D2 — Costo de singles de bulk / sellado abierto:** CERRADO. Costo **por carta**, distribuyendo el total del lote ponderado por su **precio de venta**. El costo se ingresa en la compra; nunca sale del CSV de ManaBox (ver §5.2).
...
## 10. Plan de construcción (sub-tandas verificables)

El bloque es grande; se construye en cuatro sub-tandas en orden de dependencia, cada una verificable sola antes de pasar a la siguiente (disciplina B1).

- **2A — Catálogo base.** Modelo de producto (4 categorías), costo, precio, margen calculado, y la tabla `Settings`. *Verificable:* crear un producto de cada tipo y ver su margen; editar un multiplicador en Settings.
- **2B — Motor de inventario.** Los dos modos de rastreo + contrato `reservar → (confirmar | liberar)` bajo candado, con TTL y proceso de expiración. *Verificable:* reservar, confirmar, liberar; doble reserva sobre la última unidad rechazada; reserva vencida liberada sola.
- **2C — Importación ManaBox.** Parseo del CSV, lógica heredada (SKU, Scryfall, mapeos, Foil), idempotencia. *Verificable:* importar `verde.csv` → 264 cartas con su precio de venta; reimportar no duplica.
- **2D — Costo en la compra.** Ingreso de bulk / sellado con distribución ponderada por precio de venta, y trade × 400. *Verificable:* cargar un bulk → costo repartido y margen por carta.
